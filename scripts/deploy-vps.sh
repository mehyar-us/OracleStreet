...
ensure_secret ORACLESTREET_PG_REPOSITORIES 'printf contacts,suppressions,templates,campaigns,send_queue,email_events,users,user_invite_password_workflow,admin_sessions,audit_log,warmup_policies,reputation_policies,data_sources,data_source_encrypted_secrets,data_source_import_schedules,controlled_live_test_proof_audits'
pg_repositories="$(grep '^ORACLESTREET_PG_REPOSITORIES=' /etc/oraclestreet/oraclestreet.env | tail -n1 | cut -d= -f2-)"
for repo in contacts suppressions templates campaigns send_queue email_events users user_invite_password_workflow admin_sessions audit_log warmup_policies reputation_policies data_sources data_source_encrypted_secrets data_source_import_schedules controlled_live_test_proof_audits; do
