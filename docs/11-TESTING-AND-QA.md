...
- PostgreSQL repository readiness: `GET /api/database/repositories` must require admin auth, expose migration `004_policy_repository_foundation`, list module-to-table readiness for contacts/suppressions/templates/campaigns/send queue/events/warm-up/reputation/audit/users/remote import schedules/controlled proof audits, audit the view, avoid secrets, avoid probes, and keep live repository writes disabled until the runtime adapter is enabled.
...
- Campaign reporting safe summary: `GET /api/campaigns/reporting` must require admin auth, summarize per-campaign dry-run queued/dispatched jobs, internal events, open/click engagement counts/rates, unsubscribe counts, and affiliate metadata summaries, audit the view, and keep `realDeliveryAllowed: false`.
- Campaign affiliate metadata and audit timeline: campaign draft creation may capture affiliate program, offer ID, payout model, tracking-template configured flag, UTM source/campaign, and planning notes. `GET /api/campaigns/affiliate-summary` and `GET /api/campaigns/audit-timeline` must require admin auth, return no secrets, mutate no users/queues/providers, audit views, and keep `realDeliveryAllowed: false`.
...
- Reporting dashboard depth: `GET /api/email/reporting/dashboard` must require admin auth, return aggregate campaign/source/domain/trend/queue rollups, audit dashboard views, include no secrets, avoid network probes/delivery mutations, and keep `realDeliveryAllowed: false`.
...
- Data source registry baseline: `GET`/`POST /api/data-sources` must require admin auth, accept only PostgreSQL source URLs, redact passwords, skip network probes, keep `syncEnabled: false`, audit create/list actions, persist source registry metadata through local PostgreSQL on VPS when `ORACLESTREET_PG_REPOSITORIES` includes `data_sources`, keep safe in-memory fallback for tests/adapter failure, and never pull remote data.
- Encrypted data source secret baseline: `POST /api/data-sources` with `storeSecret: true` must require `ORACLESTREET_DATA_SOURCE_SECRET_KEY`, store only encrypted connection material with AES-256-GCM, persist encrypted secret ciphertext metadata through local PostgreSQL on VPS when `ORACLESTREET_PG_REPOSITORIES` includes `data_source_encrypted_secrets`, return only redacted URL metadata plus an encrypted secret ref, and never expose plaintext secrets in create/list/error responses.
...
- Remote import schedule persistence: `GET`/`POST /api/data-source-import-schedules` must require admin auth, validate SELECT-only/bounded recurring import plans, persist through local PostgreSQL on VPS when `ORACLESTREET_PG_REPOSITORIES` includes `data_source_import_schedules`, keep in-memory fallback for tests/adapter failure, perform no immediate remote pull or worker start, expose no secrets, and keep `realDeliveryAllowed: false`.
...
- Policy repository foundation migration: `GET /api/schema/migrations` must list `004_policy_repository_foundation`, which creates `warmup_policies`, `reputation_policies`, and `repository_migration_status` for the PostgreSQL persistence migration path without storing secrets or applying migrations from the API. `008_policy_runtime_status` records policy runtime repository readiness metadata without storing secrets or applying migrations from the API. `009_schedule_proof_runtime` creates runtime tables for remote import schedules and controlled proof audits without storing secrets or applying migrations from the API.
- RBAC enforcement hardening: `GET /api/platform/rbac-policy` must require admin auth and expose route permission policy safely. Hardened protected routes such as admin-user directory/invite planning, audit log access, contact imports, and data-source write/schedule paths must reject insufficient roles with `403 forbidden`, required permission metadata, an audited `rbac_permission_denied` event, no user/role mutation, no secret output, and `realDeliveryAllowed: false`.
- Campaign repository runtime migration: `GET /api/schema/migrations` must list `005_campaign_repository_runtime_ids`, which relaxes campaign segment/template IDs to text and adds dry-run campaign state metadata columns for the PostgreSQL runtime repository path without storing secrets or applying migrations from the API. It must also list `010_campaign_affiliate_metadata`, which adds safe campaign affiliate/planning metadata fields without storing secrets or applying migrations from the API.
