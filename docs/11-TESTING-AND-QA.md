...
- Frontend warm-up/reputation workflow: `/` after admin login and reputation-gate load must show visible sender-domain warm-up controls wired to `/api/email/warmup/plan` plus auto-pause threshold save/evaluate controls wired to `/api/email/reputation/policy` and `/api/email/reputation/auto-pause`, render accepted/rejected feedback, and avoid external writes or delivery unlocks.
