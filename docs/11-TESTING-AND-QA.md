...
- Frontend warm-up planner workflow: `/` after admin login and reputation-gate load must show visible sender-domain warm-up controls wired to `/api/email/warmup/plan`, render accepted/rejected preview feedback, and avoid external writes or delivery unlocks.
