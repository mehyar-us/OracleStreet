...
- Frontend remote PostgreSQL query workflow: `/` after admin login must show a visible SELECT-only query validator, gated live execution controls, and contact import preview mapper in the remote PostgreSQL panel; call `/api/data-source-query/validate`, `/api/data-source-query/execute`, `/api/data-source-schema/discover`, and `/api/data-source-import/preview`; report accepted/blocked/preview status, avoid plaintext secrets, and keep live remote execution/import mutation disabled unless backend gates pass.
