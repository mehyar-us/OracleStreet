...
- Warm-up policy and schedule-cap enforcement: `GET`/`POST /api/email/warmup/policy` and `POST /api/email/warmup/schedule-cap` must require admin auth, validate sender-domain policy inputs, persist policy through local PostgreSQL on VPS when `ORACLESTREET_PG_REPOSITORIES` includes `warmup_policies`, audit view/save/evaluate actions, reject over-cap campaign dry-run schedules with `warmup_daily_cap_exceeded`, mutate no queues/providers/DNS, and keep `realDeliveryAllowed: false`. Tests/local adapter failure keep safe in-memory fallback.
