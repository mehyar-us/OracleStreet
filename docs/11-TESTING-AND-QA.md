...
- Frontend remote PostgreSQL query workflow: `/` after admin login must show a visible SELECT-only query validator and gated live execution controls in the remote PostgreSQL panel, call `/api/data-source-query/validate`, `/api/data-source-query/execute`, and `/api/data-source-schema/discover`, report accepted/blocked status, avoid plaintext secrets, and keep live remote execution disabled unless backend env and exact approval gates pass.
