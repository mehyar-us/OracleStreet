...
- RBAC operations review board showing user/session posture, route coverage, pending invite/reset follow-up, and no user/role/session mutation.
...
- Users/RBAC UI shows read-only role-change impact previews before role updates, including permission/surface deltas, guardrails, and session-revocation implications without mutating users or sessions.
...
- Controlled live-test final approval packet with pass/block rows, evidence checklist, max-one-message boundary, and no-send/no-mutation guarantees.
...
- Reporting operations digest with executive cards, priority source/domain/campaign risk rows, next-best actions, and read-only/no-mutation safety.
...
- Contact browser search/filter API/UI:
  ...
  - read-only source-quality remediation board combining hygiene, quarantine, and source × domain gates into prioritized operator actions
  - read-only contact source operations digest combining remediation, quarantine, risk triage, and audience readiness into next-best actions
  - read-only source quarantine plans with operator checklists and no source/contact/segment mutation
  ...
  - read-only domain risk plans with blocked/ready counts, MX-probe-free safety, and warm-up allocation recommendations
  - read-only engagement recency plans with recently engaged/dormant/no-positive-engagement cohorts and no queue mutation
  - read-only consent provenance reviews with consent/source/issue rows and no contact mutation
  - read-only source-detail reviews for import batch/form provenance gaps and no contact mutation
...
- Campaign calendar warm-up cap review board that combines cap rows, campaign readiness, gates, and operator actions without schedule/queue mutation
  ...
  - read-only repermission plan for stale/missing-consent contacts with do-not-contact guardrails and no outbound repermission sends
  - read-only audience exclusion preview that shows retained/excluded contacts and exclusion reasons before segment snapshot use without mutating segments
