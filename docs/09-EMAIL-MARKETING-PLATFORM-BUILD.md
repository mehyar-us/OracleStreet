...
The root frontend now exposes a protected admin CMS workbench after login. It loads live safe-read panels for contacts, templates, campaigns, send queue, suppressions, remote PostgreSQL sources/sync dry-runs, reputation/readiness gates, and audit events. The contacts module now includes a visible consent/source import workflow with validate-only and import actions backed by `/api/contacts/import/validate` and `/api/contacts/import`. Remaining panels are intentionally read/dry-run first while PostgreSQL persistence and safety gates are completed; the next frontend step is turning templates, campaigns, queue, and remote source panels into full CRUD/action workflows.
