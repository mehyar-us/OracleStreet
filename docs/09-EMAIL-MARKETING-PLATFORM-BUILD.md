...
The root frontend now exposes a protected admin CMS workbench after login. It loads live safe-read panels for contacts, templates, campaigns, send queue, suppressions, remote PostgreSQL sources/sync dry-runs, reputation/readiness gates, and audit events. The contacts module includes a visible consent/source import workflow with validate-only and import actions backed by `/api/contacts/import/validate` and `/api/contacts/import`. The templates module includes a visible safe draft creator wired to `/api/templates` and `/api/templates/preview`, preserving unsubscribe-language validation and no-delivery preview. The campaigns module now includes a visible dry-run builder wired to estimate, create draft, approve, schedule, and enqueue dry-run endpoints while keeping real delivery disabled. Remaining panels are intentionally read/dry-run first while PostgreSQL persistence and safety gates are completed; the next frontend step is turning queue and remote source panels into full CRUD/action workflows.
