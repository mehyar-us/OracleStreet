...
import { campaignReportingSummary, emailReportingSummary, reportingDashboardDepth, reportingDashboardDrilldown, reportingDeliverabilityAudit, reportingExportPreview, reportingOperationsDigest, sendingReadinessSummary } from './lib/reporting.js';
...
export const createHandler = () => {
  return async (req, res) => {
    ...
    if (url.pathname === '/api/data-source-import-schedules/operations-board' || url.pathname === '/data-source-import-schedules/operations-board') {
      if (!requireMethod(req, res, 'GET')) return;
      const session = requirePermission(req, res, 'manage_data_sources');
      if (!session) return;
      const result = planDataSourceImportSchedulerOperations(Object.fromEntries(url.searchParams.entries()));
      recordAuditEvent({ action: 'data_source_import_scheduler_operations_view', actorEmail: session.email, status: 'ok', details: { schedules: result.totals.schedules, operationRows: result.totals.operationRows, dueSchedules: result.totals.dueSchedules, blockedSchedules: result.totals.blockedSchedules, noWorkerStarted: true, noRemoteConnectionOpened: true, rowsPulled: 0, contactsMutated: 0, realDeliveryAllowed: false } });
      return jsonResponse(res, 200, result);
    }
    ...
    if (url.pathname === '/api/email/reporting/operations-digest' || url.pathname === '/email/reporting/operations-digest') {
      if (!requireMethod(req, res, 'GET')) return;
      const session = requireSession(req, res);
      if (!session) return;
      const result = reportingOperationsDigest();
      recordAuditEvent({ action: 'reporting_operations_digest_view', actorEmail: session.email, status: 'ok', details: { priorityRows: result.priorityRows.length, highRiskSources: result.executiveCards.highRiskSources, highRiskDomains: result.executiveCards.highRiskDomains, aggregateOnly: true, noQueueMutation: true, noProviderMutation: true, noSuppressionMutation: true, realDeliveryAllowed: false } });
      return jsonResponse(res, 200, result);
    }
    ...
    if (url.pathname === '/api/email/controlled-live-test/operator-actions' || url.pathname === '/email/controlled-live-test/operator-actions') {
      if (!requireMethod(req, res, 'GET')) return;
      const session = requireSession(req, res);
      if (!session) return;
      const result = planControlledLiveTestOperatorActions();
      recordAuditEvent({ action: 'email_controlled_live_test_operator_actions_view', actorEmail: session.email, target: 'controlled-live-test', status: 'ok', details: { actionRows: result.totals.actionRows, readinessBlockers: result.totals.readinessBlockers, proofGaps: result.totals.proofGaps, noSend: true, noNetworkProbe: true, noQueueMutation: true, realDeliveryAllowed: false } });
      return jsonResponse(res, 200, result);
    }
    ...
    if (url.pathname === '/api/contacts/source-quality-remediation-board' || url.pathname === '/contacts/source-quality-remediation-board') {
      if (!requireMethod(req, res, 'GET')) return;
      const session = requireSession(req, res);
      if (!session) return;
      const result = contactSourceQualityRemediationBoard(Object.fromEntries(url.searchParams.entries()));
      recordAuditEvent({ action: 'contact_source_quality_remediation_board_view', actorEmail: session.email, status: 'ok', details: { scoreThreshold: result.scoreThreshold, sourcesReviewed: result.totals.sourcesReviewed, highPrioritySources: result.totals.highPrioritySources, quarantineRecommended: result.totals.quarantineRecommended, noContactMutation: true, noSegmentMutation: true, realDeliveryAllowed: false } });
      return jsonResponse(res, 200, result);
    }
    ...
    if (url.pathname === '/api/campaigns/calendar/launch-readiness' || url.pathname === '/campaigns/calendar/launch-readiness') {
      if (!requireMethod(req, res, 'GET')) return;
      const session = requireSession(req, res);
      if (!session) return;
      const result = campaignCalendarLaunchReadiness(Object.fromEntries(url.searchParams.entries()));
      recordAuditEvent({ action: 'campaign_calendar_launch_readiness_view', actorEmail: session.email, status: 'ok', details: { windowDays: result.windowDays, campaigns: result.totals.campaigns, blockedCampaigns: result.totals.blockedCampaigns, automaticScheduleMutationAllowed: false, automaticQueueMutationAllowed: false, realDelivery: false } });
      return jsonResponse(res, 200, result);
    }
